...
pygame.display.set_caption("How fast you can type?")
...
while True:
    ...
    current = time.clock()
    speed = score
    if seconds-current+clock_start < 0:
        game_over = True
    elif current <= 10:
    ...
    print_text(font1, 0, 20, "Try to keep up for 10 second...")

    if key_flag:
        print_text(font1, 500, 0, "<key>")
    if not game_over:
        print_text(font1, 0, 80, "Time: " + str(int(seconds-current+clock_start)))
